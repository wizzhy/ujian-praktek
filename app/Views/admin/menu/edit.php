...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            <nav class="flex items-center space-x-2 text-xs text-slate-500 mb-1">
                ...
                <span class="text-slate-800 font-semibold">Edit Menu: <?= esc($menu['name']) ?></span>
            </nav>
            <h1 class="font-display font-extrabold text-2xl text-slate-900 tracking-tight">Perbarui Data Menu Makanan</h1>
        </div>
    ...
    <div class="bg-white rounded-3xl border border-slate-200 shadow-sm p-6 sm:p-8">
        <form action="<?= base_url('admin/menu/update/' . $menu['id']) ?>" method="POST" enctype="multipart/form-data" class="space-y-6">
            <?= csrf_field() ?>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-6">
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="number" id="price" name="price" value="<?= old('price', (int)$menu['price']) ?>" required min="0" step="500"
                ...
                <div>
                    ...
                    <select id="spiciness_level" name="spiciness_level"
                        ...
                        <option value="0" <?= old('spiciness_level', $menu['spiciness_level']) == '0' ? 'selected' : '' ?>>Level 0 (Tidak Pedas)</option>
                        <option value="1" <?= old('spiciness_level', $menu['spiciness_level']) == '1' ? 'selected' : '' ?>>Level 1 (Pedas Sedang)</option>
                        <option value="2" <?= old('spiciness_level', $menu['spiciness_level']) == '2' ? 'selected' : '' ?>>Level 2 (Pedas Mantap)</option>
                        <option value="3" <?= old('spiciness_level', $menu['spiciness_level']) == '3' ? 'selected' : '' ?>>Level 3 (Ekstra Pedas Manado)</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="number" id="calories" name="calories" value="<?= old('calories', $menu['calories']) ?>" min="0"
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="3" required
                          class="w-full px-4 py-3 bg-slate-50 border <?= isset(session('errors')['description']) ? 'border-rose-500 bg-rose-50/30' : 'border-slate-200' ?> rounded-2xl text-sm focus:outline-none focus:ring-2 focus:ring-primary focus:bg-white transition-all"><?= old('description', $menu['description']) ?></textarea>
            ...
            <div>
                ...
                <input type="text" id="ingredients" name="ingredients" value="<?= old('ingredients', $menu['ingredients']) ?>" required
            ...
            <!-- Baris 5: Foto Menu Saat Ini & Pilihan Ganti -->
            <div class="p-4 rounded-2xl bg-purple-50/50 border border-purple-100 flex flex-col md:flex-row items-center gap-6">
                <!-- Preview Foto Saat Ini -->
                <div class="flex items-center space-x-4">
                    <div class="w-16 h-16 rounded-2xl overflow-hidden bg-slate-200 border border-slate-300 flex-shrink-0">
                        <?php 
                        ...
                        ?>
                        <img src="<?= $imgSrc ?>" alt="<?= esc($menu['name']) ?>" class="w-full h-full object-cover">
                    </div>
                    <div>
                        <span class="text-xs font-bold text-slate-800 block">Foto Saat Ini</span>
                        <span class="text-[11px] text-slate-500 block truncate max-w-[150px]"><?= esc($menu['image'] ?? 'Default') ?></span>
                    </div>
                </div>
                ...
                <div class="flex-1 w-full grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        <label class="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-1">
                            Ganti dengan Upload File Baru
                        </label>
                        <input type="file" name="image" accept="image/*"
                               class="w-full text-xs text-slate-500 file:mr-3 file:py-2 file:px-3 file:rounded-xl file:border-0 file:text-xs file:font-semibold file:bg-primary file:text-white hover:file:bg-primary-dark cursor-pointer transition-all">
                    </div>

                    <!-- Atau Pilih Dari Galeri AI -->
                    <div>
                        <label for="image_select" class="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-1">
                            Atau Pilih Dari Galeri AI
                        </label>
                        <select id="image_select" name="image_select" class="w-full px-3 py-2 bg-white border border-slate-200 rounded-xl text-xs text-slate-700 focus:outline-none focus:ring-2 focus:ring-primary">
                            <option value="">-- Tetap Gunakan Foto Saat Ini --</option>
                            <option value="tinutuan_original.jpg">Tinutuan Original Spesial</option>
            ...
            <div class="flex flex-wrap items-center gap-6 pt-2">
                <label class="flex items-center space-x-3 cursor-pointer">
                    <input type="checkbox" name="is_available" value="1" <?= old('is_available', $menu['is_available']) ? 'checked' : '' ?> class="w-5 h-5 text-primary rounded-lg border-slate-300 focus:ring-primary">
                    <span class="text-sm font-semibold text-slate-800">Menu Tersedia untuk Dipesan</span>
                </label>

                <label class="flex items-center space-x-3 cursor-pointer">
                    <input type="checkbox" name="is_favorite" value="1" <?= old('is_favorite', $menu['is_favorite']) ? 'checked' : '' ?> class="w-5 h-5 text-accent rounded-lg border-slate-300 focus:ring-accent">
